  ...
  while ( (key = dynamic_cast<TKey*>(itkey.Next())) ) {
    ...
    if (obj->InheritsFrom("TDirectory")) {
      ...
      if ( (sscanf(indir2->GetName(),"Eta_%f-%f",&etamin,&etamax)==2) and (etamax>etamin) ) {
        ...
        loclvl++;
        cout << " trigger: " << indir2->GetName() << endl;
      }
      if (loclvl==1) cout << endl << "Entering: " << indir2->GetName() << endl;
    ...
    if (obj->InheritsFrom("TH1")) { // Normalize (or just pass on) histogrammish objects
      ...
      if (_jp_isdt) {
        ...
        if (_jp_usetriglumi) {
          ...
          TH1D* lumihistoref = dynamic_cast<TH1D*>(indir->Get("../jt450/hlumi"));
          assert(lumihisto);
          assert(lumihistoref);
      ...
      // CAUTION: All MC content and for DT Profiles and such are not weighted by purpose
      outdir->cd();
      obj2->Write();
      obj2->Delete();
      indir->cd();
